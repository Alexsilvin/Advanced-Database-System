            ...
            <div className="relative h-100 md:h-125">
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <div
                            className="absolute bottom-12 left-8 md:left-12 max-w-2xl space-y-4 cursor-pointer text-left"
                            onClick={() => onSelect(currentGame)}
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                    e.preventDefault();
                                    onSelect(currentGame);
                                }
                            }}
                            role="button"
                            tabIndex={0}
                            aria-label={`Open details for ${currentGame.title}`}
                            title={`Open details for ${currentGame.title}`}
                        >
                            <motion.span
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: 0.2 }}
                                className="inline-block px-3 py-1 bg-neon-magenta text-white text-[10px] font-black uppercase tracking-widest -skew-x-12"
                            >
                                Featured Protocol
                            </motion.span>
                            <motion.h2
                            ...
                            >
                                <button
                                    ...
                                    disabled={isOwned}
                                    aria-label={isOwned ? `${currentGame.title} already owned` : `Acquire ${currentGame.title}`}
                                    title={isOwned ? `${currentGame.title} already owned` : `Acquire ${currentGame.title}`}
            ...
            <div className="absolute bottom-6 right-8 flex gap-2">
                {featuredGames.map((_, idx) => (
                    <button
                        ...
                        aria-label={`Go to featured game ${idx + 1}`}
                        title={`Go to featured game ${idx + 1}`}
